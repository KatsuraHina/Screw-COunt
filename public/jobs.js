...
// --- Insights ----------------------------------------------------------------
// Every insight keeps trusses (m/h) and walls (screws/h) rates separate — the
// units are incompatible — and weights by worker-hours the same way
// summarizeWorkerJobs does: per-worker units (totalUnits / workerCount) summed
// against the jobs' worked minutes, so long jobs count more than short ones.

function createTypeAccumulator() {
  return { jobs: 0, truss: { units: 0, minutes: 0 }, wall: { units: 0, minutes: 0 } };
}

function accumulateTyped(acc, job) {
  const bucket = job.jobType === "walls" ? acc.wall : acc.truss;
  acc.jobs += 1;
  bucket.units += job.totalUnits / getWorkerCount(job);
  bucket.minutes += job.netWorkedMinutes;
}

// Rates from an accumulator; null (not 0) when a type has no worked time so
// "no data" is distinguishable from "produced nothing".
function typedRates(acc) {
  return {
    trussRate: acc.truss.minutes > 0 ? acc.truss.units / (acc.truss.minutes / 60) : null,
    wallRate: acc.wall.minutes > 0 ? acc.wall.units / (acc.wall.minutes / 60) : null
  };
}

// Rank by truss rate when either side has one, otherwise by wall rate.
function compareByRate(a, b) {
  if (a.trussRate !== null || b.trussRate !== null) {
    return (b.trussRate ?? -1) - (a.trussRate ?? -1);
  }
  return (b.wallRate ?? -1) - (a.wallRate ?? -1);
}

function jobCrewIds(job) {
  return Array.isArray(job.assignedWorkerIds) ? job.assignedWorkerIds : [];
}

// Ranked co-workers of `workerId`: the rate achieved on jobs they shared.
// Partner names come from the jobs' own assignedWorkers so partners removed
// from the roster still rank.
export function analyzePartnerStats(jobs, workerId) {
  const partners = new Map();

  jobs.forEach((job) => {
    const ids = jobCrewIds(job);
    if (ids.length < 2 || !ids.includes(workerId)) {
      return;
    }
    (job.assignedWorkers || []).forEach((worker) => {
      if (!worker?.id || worker.id === workerId) {
        return;
      }
      const entry = partners.get(worker.id) ?? {
        partnerId: worker.id,
        partnerName: worker.name || "Unknown",
        acc: createTypeAccumulator()
      };
      accumulateTyped(entry.acc, job);
      partners.set(worker.id, entry);
    });
  });
  ...
  return Array.from(partners.values())
    .map((entry) => ({
      ...
      jobsTogether: entry.acc.jobs,
      ...typedRates(entry.acc)
    }))
    .sort(compareByRate);
}

// The worker's output rate working alone vs in a crew.
export function analyzeSoloVsTeam(jobs, workerId) {
  const solo = createTypeAccumulator();
  const team = createTypeAccumulator();
  ...
  jobs.forEach((job) => {
    ...
    accumulateTyped(ids.length === 1 ? solo : team, job);
  });

  return {
    solo: { jobs: solo.jobs, ...typedRates(solo) },
    team: { jobs: team.jobs, ...typedRates(team) }
  };
}

// Ranked crews (exact membership, order ignored) by the per-worker rate their
// shared jobs achieved. Labels come from the jobs' own assignedWorkers.
export function analyzeCrewCombos(jobs) {
  ...
  jobs.forEach((job) => {
    ...
    const entry = combos.get(key) ?? {
      label: (job.assignedWorkers || []).map((worker) => worker.name).filter(Boolean).join(" & "),
      acc: createTypeAccumulator()
    };
    accumulateTyped(entry.acc, job);
  ...
  return Array.from(combos.values())
    .map((entry) => ({ label: entry.label, jobs: entry.acc.jobs, ...typedRates(entry.acc) }))
    .sort(compareByRate);
}

// Personal bests over the given (already worker-filtered) jobs, per job type:
// the biggest shift-day (per-worker units summed across the day's jobs) and the
// single job with the highest stored per-worker rate.
export function analyzePersonalBests(jobs) {
  const bests = {
    truss: { bestDay: null, bestRateJob: null },
    wall: { bestDay: null, bestRateJob: null }
  };
  const dayUnits = { truss: new Map(), wall: new Map() };

  jobs.forEach((job) => {
    const typeKey = job.jobType === "walls" ? "wall" : "truss";
    const dayKey = getShiftDayKey(job);
    const units = job.totalUnits / getWorkerCount(job);
    dayUnits[typeKey].set(dayKey, (dayUnits[typeKey].get(dayKey) ?? 0) + units);

    const rate = Number(job.rate) || 0;
    if (rate > 0 && (!bests[typeKey].bestRateJob || rate > bests[typeKey].bestRateJob.rate)) {
      bests[typeKey].bestRateJob = { rate, dayKey };
    }
  });

  ["truss", "wall"].forEach((typeKey) => {
    dayUnits[typeKey].forEach((units, dayKey) => {
      if (units > 0 && (!bests[typeKey].bestDay || units > bests[typeKey].bestDay.units)) {
        bests[typeKey].bestDay = { dayKey, units };
      }
    });
  });
...
// Is the output rate rising or falling: the last 14 days vs the 14 before
// that. Direction is null when either window has no data; "flat" within ±2%.
export function analyzeTrend(jobs, now = new Date()) {
  const DAY_MS = 24 * 60 * 60 * 1000;
  const recentStart = now.getTime() - 14 * DAY_MS;
  const priorStart = now.getTime() - 28 * DAY_MS;
  const recent = createTypeAccumulator();
  const prior = createTypeAccumulator();

  jobs.forEach((job) => {
    const ended = new Date(job.endedAt).getTime();
    if (!Number.isFinite(ended) || ended < priorStart || ended > now.getTime()) {
      return;
    }
    accumulateTyped(ended >= recentStart ? recent : prior, job);
  });

  const recentRates = typedRates(recent);
  const priorRates = typedRates(prior);

  const describe = (recentRate, priorRate) => {
    if (recentRate === null || priorRate === null) {
      return { recentRate, priorRate, direction: null };
    }
    let direction = "flat";
    if (recentRate > priorRate * 1.02) {
      direction = "up";
    } else if (recentRate < priorRate * 0.98) {
      direction = "down";
    }
    return { recentRate, priorRate, direction };
  };

  return {
    truss: describe(recentRates.trussRate, priorRates.trussRate),
    wall: describe(recentRates.wallRate, priorRates.wallRate)
  };
}
